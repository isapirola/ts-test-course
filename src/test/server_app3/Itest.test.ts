...
describe("Server app integration tests", () => {
    ...
    it("should create and retrieve multiple reservations if authorized", async () => {
        ...
        const getAllReservations = await fetch(`http://localhost:8080/reservation/all`, {
            method: HTTP_METHODS.GET,
            headers: {
                authorization: token,
            },
        ...
        const resultBody = await getAllReservations.json();
        expect(getAllReservations.status).toBe(HTTP_CODES.OK);
